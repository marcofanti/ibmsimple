...
Monitor.prototype = {
    ...
    startMonitor: function() {
        var i, j, thisForm, fields, field, hiddenField, forms = document.getElementsByTagName("label");


        console.log('*******************      form = ' + forms);
        if (null == document.getElementById(bw.behavio_hidden_id) || void 0 === document.getElementById(bw.behavio_hidden_id))
            for ((hiddenField = document.createElement("input")).setAttribute("type", "hidden"), hiddenField.setAttribute("name", bw.behavio_hidden_id), hiddenField.setAttribute("id", bw.behavio_hidden_id), i = 0; i < forms.length; i++)(thisForm = forms[i]).appendChild(hiddenField);
        for ("undefined" != typeof jQuery && jQuery("form").submit(bw.submitHandler), i = 0; i < forms.length; i++) {
            ...
            for (fields = thisForm.getElementsByTagName("input"), j = 0; j < fields.length; j++) 
            console.log('*******************      fields = ' + fields);

                "checkbox" !== (field = fields[j]).type && "radio" !== field.type && "hidden" !== field.type && "button" !== field.type && "submit" !== field.type && -1 === this.ignoreFields.names.indexOf(field.name) && -1 === this.ignoreFields.ids.indexOf(field.id) && (field.addEventListener ? (field.addEventListener("keydown", this.keyHandler, !1), field.addEventListener("keyup", this.keyHandler, !1), this.isAndroid && !this.isFirefox && this.fallbackListeners(field)) : field.attachEvent && (field.attachEvent("onkeydown", this.keyHandler), field.attachEvent("onkeyup", this.keyHandler)))
